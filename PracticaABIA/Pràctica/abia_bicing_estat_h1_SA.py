"""
FITXER ESTAT PER LA PRIMERA HEURÍSTICA I SIMULATED ANNEALING
"""
...
class Estat(object):
    ...
    def copia(self) -> Estat:
        ...
        #furgonetes_util_copia = copy.deepcopy(self.furgonetes_util)
    ...
    def generate_actions(self) -> Generator[BicingOperator, None, None]:
        """ 
        Generem una acció pel simulated annealing de manera aleatoria
        """
       
        estacio_origen_combinacions = []      
        for id_furgoneta in range(len(self.furgonetes_util)):
            for id_est in range(len(self.params.estaciones.lista_estaciones)):
                if id_est not in self.estacions_origen and not any(id_est == desti[0] for desti in self.furgonetes_util[id_furgoneta][1]):
                    estacio_origen_combinacions.append([id_est, id_furgoneta])                  
                        
        estacio_desti_combinacions = []                               
        for id_furgoneta in range(len(self.furgonetes_util)):
            for id_desti in range(len(self.params.estaciones.lista_estaciones)):                
                ...
                if len(self.furgonetes_util[id_furgoneta][1]):
                       ...
                       if id_desti != self.furgonetes_util[id_furgoneta][1][0][0]:
                            estacio_desti_combinacions.append([id_desti, id_furgoneta])
                else:
                    estacio_desti_combinacions.append([id_desti, id_furgoneta])

        afegir_desti_combinacions = []
        intercanviar_destins_combinacions = []
        for id_furgoneta in range(len(self.furgonetes_util)):
            if len(self.furgonetes_util[id_furgoneta][1]) == 1: #si la furgoneta només té assignada una estacio, afegim estació
                for id_desti in range(len(self.params.estaciones.lista_estaciones)):
                    if id_desti != self.furgonetes_util[id_furgoneta][1][0][0]:
                        afegir_desti_combinacions.append([id_desti, id_furgoneta])
                ...
                intercanviar_destins_combinacions.append([id_furgoneta])           

        eliminar_estacio_combinacions = []
        for id_furgoneta in range(len(self.furgonetes_util)):
            if len(self.furgonetes_util[id_furgoneta][1]): #si te estacio desti    
                if len(self.furgonetes_util[id_furgoneta][1]) == 2: #si visita dos estacions
                    eliminar_estacio_combinacions.append([id_furgoneta]) #eliminem primer només el segon desti
                    eliminar_estacio_combinacions.append([id_furgoneta, True]) #eliminem els dos destins
                else: #visita una estacio
                    eliminar_estacio_combinacions.append([id_furgoneta])

        pivotar_destins_combinacions = []
        for id_furgoneta in range(len(self.furgonetes_util)):
            if len(self.furgonetes_util[id_furgoneta][1]) > 1:
                ...
                for i in range(len(self.params.estaciones.lista_estaciones)):
                    pivotar_destins_combinacions.append([id_furgoneta, 0, i])

                for i in range(len(self.params.estaciones.lista_estaciones)):
                    pivotar_destins_combinacions.append([id_furgoneta, 1, i])

        # Definim les longituds dels conjunts
        n1 = len(estacio_origen_combinacions)
        n2 = len(estacio_desti_combinacions)
        n3 = len(afegir_desti_combinacions)
        n4 = len(intercanviar_destins_combinacions)
        n5 = len(eliminar_estacio_combinacions)
        n6 = len(pivotar_destins_combinacions)

        # Calcula un valor aleatori entre 0 i 1
        random_value = random.random()

        # Escollim una combinació basada en probabilitats:
        if random_value < (n1 / (n1 + n2 + n3 + n4 + n5 + n6)):
            # Escollim una combinació de estacio_origen_combinacions
            combination = random.choice(list(estacio_origen_combinacions))
            yield Estacio_origen(combination[0], combination[1])
            
        elif random_value < ((n1 + n2) / (n1 + n2 + n3 + n4 + n5 + n6)):
            # Escollim una combinació de estacio_desti_combinacions
            combination = random.choice(list(estacio_desti_combinacions))
            yield Estacio_desti(combination[0], combination[1])
            
        elif random_value < ((n1 + n2 + n3) / (n1 + n2 + n3 + n4 + n5 + n6)):
            # Escollim una combinació de afegir_desti_combinacions
            combination = random.choice(list(afegir_desti_combinacions))
            yield Afegir_estacio_desti(combination[0], combination[1])
            
        elif random_value < ((n1 + n2 + n3 + n4) / (n1 + n2 + n3 + n4 + n5 + n6)):
            # Escollim una combinació de intercanviar_destins_combinacions
            combination = random.choice(list(intercanviar_destins_combinacions))
            yield Intercanviar_destins(combination[0])
            
        elif random_value < ((n1 + n2 + n3 + n4 + n5) / (n1 + n2 + n3 + n4 + n5 + n6)):
            # Escollim una combinació de eliminar_estacio_combinacions
            combination = random.choice(list(eliminar_estacio_combinacions))
            if len(combination) == 2:
                yield Eliminar_estacio(combination[0], combination[1])
            else:
                yield Eliminar_estacio(combination[0])
            
        else:
            # Escollim una combinació de pivotar_destins_combinacions
            combination = random.choice(list(pivotar_destins_combinacions))
            yield Pivotar_destins(combination[0], combination[1], combination[2])
    ...
    def __repr__(self):       
            ...
            for furgoneta in self.furgonetes_util:
                ...
                i += 1
                repr_str += f"\n"
            repr_str += f"Benefici total: {self.heuristic()}€ \n"
...
def calcul_estat_inicial(params: ProblemParameters, furgonetes: List[List[List]], bicis_en_estacions: List[List]):
    ...
    for id_furgoneta in range(len(furgonetes)):
        ...
        #4. Canviem el benefici obtingut de la furgoneta
        furgonetes[id_furgoneta][2] = benefici_furgoneta(id_furgoneta, furgonetes, bicis_en_estacions)


def benefici_furgoneta(id_furgoneta: int, furgonetes_util: List[List[List]], bicis_en_estacions: List[List]) -> float:
    """
    Aquesta funcio calcula per una furgoneta el benefici que ha obtingut
    """
